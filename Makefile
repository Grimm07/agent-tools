...
new: ## Scaffold a unit: make new KIND=<go-mcp|python-tool|project-mcp> NAME="<name>"
	@test -n "$(KIND)" || { echo 'KIND required (go-mcp|python-tool|project-mcp)'; exit 1; }
	...
	case "$(KIND)" in \
	  ...
	  *) echo "unknown KIND: $(KIND) (want go-mcp|python-tool|project-mcp)"; exit 1 ;; \
	esac; \
