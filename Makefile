...
new: ## Scaffold a unit: make new KIND=<go-mcp|python-tool> NAME="<name>"
	@test -n "$(KIND)" || { echo 'KIND required (go-mcp|python-tool)'; exit 1; }
	...
	case "$(KIND)" in \
	  ...
	  *) echo "unknown KIND: $(KIND) (want go-mcp|python-tool)"; exit 1 ;; \
	esac; \
