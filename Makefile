...
new: ## Scaffold a unit: make new KIND=<go-mcp|python-tool|project-mcp|claude-agent|skill> NAME="<name>"
	@test -n "$(KIND)" || { echo 'KIND required (go-mcp|python-tool|project-mcp|claude-agent|skill)'; exit 1; }
	...
	case "$(KIND)" in \
	  ...
	  *) echo "unknown KIND: $(KIND) (want go-mcp|python-tool|project-mcp|claude-agent|skill)"; exit 1 ;; \
	esac; \
	if [ -e "$$dest" ]; then echo "$$dest already exists"; exit 1; fi; \
